# Gridiron - post-update questions review bundle

_Generated 2026-06-22T23:58Z by scripts/build-review-questions-bundle.sh. This ONE file is self-contained: the focused review brief is first, then every referenced source file inline under `===== FILE: <path> =====` markers. You do not need the repo to review._
...
> The brief's file list is satisfied below. Each file appears between `===== FILE: <path> =====` and `===== END FILE =====` markers, in priority order.
...
===== FILE: docs/REVIEW_3_CODEX_HANDOFF.md =====
...
===== END FILE: docs/REVIEW_3_CODEX_HANDOFF.md =====
...
export default function FootballHome({ onPlay }: Props) {
  ...
  const dailyTeam = todayDaily ? TEAM_PROFILES[todayDaily.team] : null;
  ...
  return (
    <div style={{ minHeight: '100svh', display: 'flex', flexDirection: 'column', padding: '0 16px 24px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 20 }}>
        ...
        >
          {todayDaily && dailyTeam
            ? `Daily done — Replay (practice) · ${dailyTeam.shortName} · ${todayDaily.gamesWon}/${SEASON_GAMES} · ${todayDaily.score} · streak ${todayDaily.streak}`
...
export default function FootballMatch(props: MatchProps) {
  ...
  const coach = useMemo(() => firstDriveCoach(gameNumber, team, match, selectedCards, preview, target, remaining), [gameNumber, team, match, selectedCards, preview, target, remaining]);
  ...
  function runPlay() {
    ...
    setMatch((m) => {
      ...
      const base = { ...m, stacksThisMatch: stacks, groundBonusThisMatch: ground, qbRunsThisMatch: qbRuns, defPlaysThisMatch: defPlays, cleanConceptsThisMatch: cleanConcepts, bombLanded: bomb, keeperLanded: keeper, lastPlay: result, popKey: m.popKey + 1 };
      ...
      {match.status === 'playing' && (
        <>
          ...
          <div className="fb-action-bar" style={{ position: 'fixed', left: '50%', bottom: 0, transform: 'translateX(-50%)', width: 'min(100%, 480px)', zIndex: 40, padding: '10px 14px calc(10px + env(safe-area-inset-bottom))', background: 'linear-gradient(180deg, rgba(9,12,17,0.15), #090c11 18%, #090c11 100%)', borderTop: `1px solid ${FB.borderSoft}`, boxShadow: '0 -16px 32px -24px rgba(0,0,0,0.9)' }}>
            <PlayPreview result={preview} count={selectedCards.length} remaining={remaining} target={target} driveScore={match.driveScore} budgetLeft={match.budgetLeft} overBudget={overBudget} coachActive={Boolean(coach)} lastPlay={match.lastPlay} scoreBeat={visibleScoreBeat} showQuality={gameNumber === 1} />
          ...
          <div>
            <div style={{ fontSize: 11, color: FB.textFaint, letterSpacing: 1.4, fontWeight: 800, margin: '0 2px 7px' }}>YOUR HAND · TAP UP TO {MAX_PLAY_CARDS}</div>
...
function CoachCall({ coach }: { coach: Coach }) {
  ...
  return (
    <div style={{ background: '#101926', border: `1px solid ${accent}66`, borderLeft: `3px solid ${accent}`, borderRadius: 12, padding: '10px 12px' }}>
      <div style={{ fontSize: 11, color: accent, letterSpacing: 1.2, fontWeight: 900 }}>COACH'S FIRST DRIVE</div>
    ...
    <div style={{ background: live ? (good ? FB.greenSoft : '#23121a') : FB.panelSoft, border: `1px solid ${live ? (good ? '#1f6b44' : '#6b3344') : FB.borderSoft}`, borderRadius: 13, padding: 12, transition: 'background .15s' }}>
      ...
      {live && (
        <>
          ...
          <div className="fb-num" style={{ marginBottom: 7, background: '#081018', border: `1px solid ${FB.borderSoft}`, borderRadius: 9, padding: '7px 8px', textAlign: 'center', color: FB.text, fontSize: 12, fontWeight: 850 }}>
...
function firstDriveCoach(
  ...
  if (gameNumber !== 1 || match.status !== 'playing') return null;
  ...
  if (match.lastPlay && selectedCards.length === 0) {
    ...
    return {
      ...
      detail: need > 0
        ...
        : `Good. Game 1 is about repeating clean concepts, not just one perfect opener.`,
      action: 'select',
...
import { bestGridironHistoryRun, loadGridironHistory, saveGridironDailyResult, saveGridironHistoryEntry } from '../lib/gridironStorage';
import { CoachPortrait } from './coachIdentity';
import { TEAM_IDENTITY } from './teamIdentity';
